<x-app-layout>
    <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 mt-2">
        ...
        <div class="w-50 lg:w-3/4 xl:max-w-md mt-6 px-6 py-4 bg-success bg-opacity-25 overflow-hidden sm:rounded-lg">
            <p class="text-xm text-center my-3 font-semibold">Please complete the form below. Your response will be submitted to {{ config('app.name') }} for review, and then you will be granted access to Meto's database.</p>
            ...
            <form method="POST" action="{{ route('signup.uni.store') }}">
                ...
                <div class="my-4 border border-secondary bg-light rounded-md p-2">
                    <x-inputs.textarea label="How did you hear about {{ config('app.name') }} and how are you looking to use {{ config('app.name') }}?" name="how"></x-inputs.textarea>
                </div>
