...
@if (Auth()->user()->role == \App\Enums\User\Role::COUNSELOR())
    <form id="notes" name="notes" action="{{ route('saveNotes') }}" method="POST" class="mt-6 bg-gray-100 shadow-md rounded-lg p-6">
        @csrf
        <div class="mb-3 text-center">
            <label for="notes" class="mb-1 text-center text-gray-800 block display-7">Notes <i class="fas fa-pencil"></i></label>
