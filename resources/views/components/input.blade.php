...
<label for="{{ $name }}" class="block font-bold text-l text-gray-700 mt-2">{{ htmlspecialchars($label) ?? $slot }}</label>
<input value="{{ $saved }}" name="{{ $name }}" id="{{ $name }}" {{ $disabled ? 'disabled' : '' }} {!! $attributes->merge([
